...
// CRIANDO O N� DO CLIENTE PARA SER CADASTRADO -----------------
...
void deletarCliente(noptrC **inicioCliente, noptrV **inicioVenda, long long int cpf)
{
    ...
    while (*inicioVenda != NULL)
    {
        ...
        if (v->venda.cpfC == cpf)
        {
            printf("Cliente presente em uma venda. Não é possível excluir.\n");
            return;
        }
...
void deletarProduto(noptrP **inicioProduto,noptrV **inicioVenda ,int codigo)
{
    ...
    while (*inicioVenda != NULL)
    {
        ...
        {
            printf("Cliente presente em uma venda. Nao e possivel excluir.\n");
            return;
        }
...
void crud_venda(noptrC **inicioCliente, noptrP **inicioProduto, noptrV **inicioVenda)
{
    ...
    do
    {
        ...
        case 6:
            printf("\n--------------------VALORES---------------------\n");
            //printf("Informe um valor para ver quais clientes compraram acima de tal valor: ");
            //scanf("%f", &valor);
...
int *existirProdutoVenda(noptrP *inicioProduto, int codProduto)
{
    ...
    printf("VOU RETORNAR AQUI, N RODA KSKSKS\n");
    return 1;
}
...
void cadastroVenda(noptrC **inicioCliente, noptrP **inicioProduto, noptrV **inicioVenda)
{
    ...
    int retornoc = existirClienteVenda(*inicioCliente, cpfCliente); // Esse aqui � para verificar se o cliente existe no CADASTRO
    if(retornoc == 1)
    {
        printf("Cliente nao encontrado!");
        return;
    ...
    int retornop = existirProdutoVenda(*inicioProduto, codigoProduto); // Esse aqui � para verificar se o produto existe no CADASTRO
    if(retornop == 1)
    {
...
// CRIA O N� PARA CRIAR UM PRODUTO INSER��O ORDENADA -------------
...
// Esse aqui � para verificar se o cliente existe no ALTERAR
...
// Esse aqui � para verificar se o produto existe no ALTERAR
...
void alterarVenda(noptrV **inicioVenda, noptrC **inicioCliente, noptrP **inicioProduto, int codVenda)
{
    ...
    else
    {
        ...
        case 3:
            ...
            // Verificar se a quantidade comprada � v�lidaaaaaaaaaaaaaaa
            /* int estoqueDisponivel = obterQuantidadeEstoque(*inicioProduto, codigoProduto);
             ...
             if (existirProdutoVenda2(inicioProduto, vend->codProduto))
             {
                 ...
                 if (quantidadeProduto >= diferenca)
                 {
                     //int diferenca = quantidadeProduto - vend->qteComprada;
                     atualizarEstoque(inicioProduto, vend->codProduto, diferenca);
                     vend->qteComprada = quantidadeProduto;
...
// Obt�m a quantidade em estoque de um produto espec�fico
int obterQuantidadeEstoque(noptrP *inicioProduto, int codProduto)
{
    ...
    while (p != NULL)
    {
        printf("valor aqui: %d\n cod_prod %d", p->produto.codigo, codProduto);
        if (p->produto.codigo == codProduto)
        {
            printf("entrou");
    ...
    // Caso o produto n�o seja encontrado, retorna -1
    printf("PASSEI AQUIN AIN\n");
    return -1;
}

void diminuirEstoque(noptrP **inicioProduto, int codProduto, int qteComprada)  // ideia de retorno
{
    noptrP *p = *inicioProduto;
...
int acimaValor(noptrV **inicioVenda, noptrP **inicioProduto)
{
    noptrV *pv = *inicioVenda;
    noptrP *pp = *inicioProduto;
    ...
    while (pv)
    {
        while (pp)
        {
            printf("Produto: %d  CodigoVenda: %d\n", pv->venda.codVenda, pp->produto.codigo);
            if (pv->venda.codVenda == pp->produto.codigo)
            {
                float valorTotal = pv->venda.qteComprada * pp->produto.preco_unitario;
                printf("\t---------------------------------------\n");
                printf("\t Valor: %f\n", valorEscolhido);
...
// Quais os produtos que est�o com a quantidade em estoque abaixo de um determinado valor escolhido pelo usu�rio
...
float obterValorTotalVenda(noptrV *venda, noptrP *inicioProduto)
{
    ...
    noptrP *produto = existirProdutoVendaValor(inicioProduto, codProduto); // DANDO ERRO AQUIIIIIIIIIIIIIIIIIII
    while (p != NULL)
    {
        printf("ENTREI SERA NO OBTER?\n");
        //int codProduto = p->venda.codProduto;
        //int quantidadeComprada = p->venda.qteComprada;

        //noptrP *produto = existirProdutoVendaValor(inicioProduto, codProduto); // EST� RETORNANDO UM INT 0 OU 1, CONSERTAR AQUIIIIIIIIIII
        if (produto != NULL)
        {
            printf("FALA QUE PASSEI POR AQUI POR FAVOR\n");
...
noptrP *existirProdutoVendaValor(noptrP *inicioProduto, int codProduto)
{
    ...
    printf("TA RETORNANDO AQUI NE PUTO\n");
    return NULL;
}
